...
export class ScoreCalculator {
    ...
    public static calculateHandValue(hand: Card[]): number {
        ...
        console.log(`%c[ScoreCalc] Calculating value for hand: [${handDesc}]`, 'color: #6495ED'); // CornflowerBlue

        if (!hand || hand.length === 0) {
            console.log(`%c[ScoreCalc]   -> Hand empty, returning 0.`, 'color: #6495ED');
            return 0;
        }
        ...
        for (const card of hand) {
            ...
            if (card.getRank() === Rank.Ace) { // Use Rank enum for comparison
                ...
                // *** ADDED Log: Ace Found ***
                console.log(`%c[ScoreCalc]   -> Found Ace. Current value: ${value}, Ace count: ${aceCount}`, 'color: #6495ED');
            } else {
                value += cardValue;
                 // *** ADDED Log: Non-Ace Found ***
                 console.log(`%c[ScoreCalc]   -> Found ${card.toString()} (${cardValue}). Current value: ${value}, Ace count: ${aceCount}`, 'color: #6495ED');
            }
        }
         // *** ADDED Log: After Initial Sum ***
         console.log(`%c[ScoreCalc]   -> Initial sum complete. Value: ${value}, Ace count: ${aceCount}`, 'color: #6495ED; font-weight: bold;');


        // Second pass: Adjust Ace value from 11 to 1 if score exceeds 21
        while (value > 21 && aceCount > 0) {
             // *** ADDED Log: Adjusting Ace ***
             console.log(`%c[ScoreCalc]   -> Value (${value}) > 21 and aceCount (${aceCount}) > 0. Adjusting Ace (11 -> 1).`, 'color: orange; font-weight: bold;');
            value -= 10; // Change one Ace from 11 to 1
            aceCount--;
             // *** ADDED Log: After Adjustment ***
             console.log(`%c[ScoreCalc]   -> Value after adjustment: ${value}, Remaining aceCount: ${aceCount}`, 'color: orange;');
        }

        // *** ADDED Log: Final Value ***
        console.log(`%c[ScoreCalc] Final calculated value: ${value}`, 'color: #6495ED; font-weight: bold;');
        return value;
    }
}
